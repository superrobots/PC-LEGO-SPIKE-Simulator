...
class Motion_sensor:
    ...
    def __init__(self):
        ...
        if self.ISDEBUG:
            print(
                "motion sensor is initialised in debug mode. Simulation motion sensor simulator type:",
                self.SIMULATORTYPE,
                " change at spike.motion_sensor.py ",
            )
    ...
    def get_yaw_angle(self):
        if self.ISDEBUG:
            print("motion_sensor get_yaw_angle() Retrieves the Hub’s yaw angle.")
        self.yaw_angle, self.yaw_angle_direction = self.simulator.get_new_value(
            issimulation=True,
    ...
    def get_roll_angle(self):
        if self.ISDEBUG:
            print("motion sensor get_roll_angle() Retrieves the Hub’s roll angle.")
        self.roll_angle, self.roll_angle_direction = self.simulator.get_new_value(
            issimulation=True,
            simulationtype=self.SIMULATORTYPE,
            isdebug=self.ISDEBUG,
    ...
    def get_pitch_angle(self):
        if self.ISDEBUG:
            print("motion sensor get_pitch_angle() Retrieves the Hub’s pitch angle.")
        self.pitch_angle, self.pitch_angle_direction = self.simulator.get_new_value(
            issimulation=True,
            simulationtype=self.SIMULATORTYPE,
            isdebug=self.ISDEBUG,
    ...
    def get_gesture(self):
        if self.ISDEBUG:
            print(
                "motion sensor get_gesture() Retrieves the most recently-detected gesture."
            )
        self.gesture_index, self.gesture_index_direction = self.simulator.get_new_value(
            issimulation=True,
            simulationtype=self.SIMULATORTYPE,
            isdebug=self.ISDEBUG,
    ...
    def get_orientation(self):
        if self.ISDEBUG:
            print(
                "motion sensor get_orientation() Retrieves the Hub's current orientation."
            )
        self.orientation_index, self.orientation_index_direction = (
            self.simulator.get_new_value(
                issimulation=True,
                simulationtype=self.SIMULATORTYPE,
                isdebug=self.ISDEBUG,
    ...
    def reset_yaw_angle(self):
        if self.ISDEBUG:
            print("motion sensor reset_yaw_angle() Sets the yaw angle to 0.")
        self.yaw_angle = 0

    def light_up_all(self, brightness=100):
        if self.ISDEBUG:
            print(
                "motion sensor light_up_all() Lights up all of the lights on the motion Sensor at the specified brightness. brightness:",
                str(brightness),
            )
    ...
    def wait_for_new_orientation(self):
        ...
        if not True:
            self.orientation_index = int(
                self.remap(self.sensor.get_values()["AcX"], 0, 1024, 0, 6)
            )
        if self.ISDEBUG:
            print(
                "motion sensor wait_for_new_orientation() Waits until the Hub’s orientation changes. Orientation at start:",
                self.orientation_array[self.orientation_index],
            )
        runwhile = True
        while runwhile:
            ...
            if self.ISDEBUG:
                print(
                    "Do changes at color sensor new reading:",
                    str(self.orientation_index),
                    " to exit it should be lower then:",
                    self.orientation_index,
                    " or it should be higher then:",
                    self.orientation_index,
                )
            if (
                self.orientation_index > old_orientation_index
                or self.orientation_index < old_orientation_index
            ):
                runwhile = False
            else:
                time.sleep(1)
    ...
    def wait_for_new_gesture(self):
        # next line is not correct implementation it should be changed later
        if not True:
            self.gesture_index = int(
                self.remap(self.sensor.get_values()["AcY"], 0, 1024, 0, 4)
            )
        if self.ISDEBUG:
            print(
                "motion sensor wait_for_new_gesture() Waits until the Hub’s gesture changes. Gesture at start:",
                self.gesture_array[self.gesture_index],
            )
        runwhile = True
        ...
        while runwhile:
            ...
            if not True:
                ...
                newreading = None
            self.gesture_index, self.gesture_index_direction = (
                self.simulator.get_new_value(
                    issimulation=True,
                    simulationtype=self.SIMULATORTYPE,
                    isdebug=self.ISDEBUG,
            ...
            if (
                self.gesture_index > old_gesture_index
                or self.gesture_index < old_gesture_index
            ):
                runwhile = False
            else:
                time.sleep(1)
    ...
    def was_gesture(self, gesture):
        if self.ISDEBUG:
            print(
                "motion sensor was_gesture(gesture) Tests whether a gesture has occurred since the last time was_gesture() was used, or since the beginning of the program (for the first use)."
            )
        self.gesture_index, self.gesture_index_direction = self.simulator.get_new_value(
            issimulation=True,
            simulationtype=self.SIMULATORTYPE,
